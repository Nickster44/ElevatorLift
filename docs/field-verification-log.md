...
| VFD serial connector | A four-pin male header, likely 2.54 mm pitch, carries VFD RX/TX. A TI `TXS0104E` appears to translate the two serial signals; no discrete transistor or series resistor was seen on this path. | Start the VFD page from a 3.3 V-to-5 V `TXS0104E` reference stage, with a defined OE pull-down and test pads. Do not assume the drive's electrical interface is safe until pinout, common/reference, idle levels, and fault behavior are bench-verified. | Part marking observed; connector pitch and pin order unconfirmed. |
...
2. Identify the VFD serial electrical specification and verify the `TXS0104E` stage on a bench fixture before connecting a new controller to the lift.
