...
assets are still useful. The schematic already uses KiCad or imported
...
September 13 hardware update: the optional AUX and MCU RUN circuits have been
removed; do not source U25 or the removed AUX parts for this revision. New R62
is a 1 kΩ 0603 resistor using standard KiCad assets, so no library download is
needed for this change. See the current hardware handoff for remaining D41
package verification and the revised pin mapping.
...
| Medium | `RV-3028-C7` | Manufacturer footprint verification and 3D model | The RTC uses `Package_SON:MicroCrystal_C7_SON-8_1.5x3.2mm_P0.9mm`, not the earlier provisional LGA footprint. Verify the exact C7 drawing and model alignment. |
...
| `TPD2EUSB30ADRTR` | Imported SnapEDA symbol/footprint at U12 on the USB data lines; verify the full suffix when sourcing. |
...
| `TSW-106-07-G-S` | J11 boot/reset header uses the KiCad 1x06, 2.54 mm footprint; pins 3/4 are NC, not UART recovery. |
...
D41 still specifies SMBJ15A with a D_SMA footprint; resolve the package mismatch
against the manufacturer drawing before ordering. Existing assets are not a
blanket package qualification.
